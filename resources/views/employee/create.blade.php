                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card card-primary">
                            ...
                            <!-- /.card-header -->
                            <!-- form start -->
                            <form role="form" action="{{route('employeeStore')}}" method="post">
                                @csrf
                                <div class="card-body">
                                    ...
                                    <input type="{{$formField->type}}" min="{{$formField->min_value}}" max="{{$formField->max_value}}" value="{{old('label_name')}}" class="form-control"
                                    name="{{$formField->name}}" id="labelName" placeholder="Enter {{$formField->label_name}}"
                                    ...
                                    @endif
                                        ...
                                        @if(($formField->type == 'radio'))
                                            ...
                                            @foreach($getOptions as $index => $option)
                                                <br><input class="custom-control-input ml-4 position-relative opacity-5 top-z-index" type="radio"
                                                           id="{{$formField->name.$index}}" name="{{$formField->name}}" value="{{$option}}"
                                                >
                                        ...
                                        @if($formField->type == 'checkbox')
                                            <input class="form-check-input ml-4"
                                                   type="checkbox" name="{{$formField->name}}" id="{{$formField->name}}">
                                        @endif
